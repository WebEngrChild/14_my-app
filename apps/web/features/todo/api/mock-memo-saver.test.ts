...
import { createMockMemoSaver } from "./mock-memo-saver";
...
describe("モック専用の送信", () => {
  test("POSTの返却IDを後続PATCHに使い、画面IDごとに作成済みを管理する", async () => {
    ...
    const save = createMockMemoSaver({
  ...
  test("実API宛ての設定ではモック用の書き込みを送らない", async () => {
    let called = false;
    const save = createMockMemoSaver({
      baseUrl: "http://localhost:3000",
      create: async () => {
        called = true;
        return { data: { id: 1 }, response: new Response() };
      },
      update: async () => {
        called = true;
    ...
    await expect(save({ localId: -1, input, isNew: true, signal })).rejects.toThrow("4010");
